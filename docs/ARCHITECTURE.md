...
credential khi frontend là SPA gọi API qua origin/port khác nhau.

| Endpoint | Yêu cầu |
|---|---|
| `POST /api/support/submit`, `GET /api/support/session/{id}` | Công khai — khách hàng ẩn danh vẫn chat được |
| `GET /api/support/tickets`, `.../accept`, `.../close` | Role `staff` hoặc `admin` |
| `GET/PUT /api/support/settings`, toàn bộ `/kb`, `/stats`, `/knowledge-gaps` | Role `admin` |
...
|---|---|---|
| Session/ticket khách hàng | Redis (`support:session:*`) | Cần persist qua restart, JSON đơn giản, không cần quan hệ phức tạp |
...
- **Tổng quan**: tổng requests, tỷ lệ tự xử lý, số ca chuyển nhân viên, biểu đồ theo giờ/loại/tự động vs escalate — tính trực tiếp từ session store thật, không mock.
...
- **Retry 3 lần + backoff cho Gemini** (`tenacity`) thay vì retry vô hạn: tránh làm khách chờ quá lâu, chấp nhận fallback an toàn (escalate) nếu lỗi kéo dài thay vì cố gọi lại mãi.
